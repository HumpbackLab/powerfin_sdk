...
choose_defconfig()
{
	...
	case $DEFCONFIG_ARRAY_LEN in
		...
		*)
			...
			local LF_BOOT_MEDIA=("sdmmc" "spinand" "emmc" "spinor")
			...
			LUNCH_NUM=$((LUNCH_NUM + 1))
			echo "${space8}${space8}[${LUNCH_NUM}] RK3506G_POWERFIN"
			LUNCH_NUM=$((LUNCH_NUM + 1))
			...
			if __IS_IN_ARRAY "$HW_INDEX" "${range_sd_card_spi_nand[@]}"; then
				...
				read -p "Which would you like? [0~1][default:0]: " BM_INDEX
				MAX_BM_INDEX=1
			elif __IS_IN_ARRAY "$HW_INDEX" "${range_powerfin[@]}"; then
				echo "${space8}${space8}[0] SD_CARD"
				echo "${space8}${space8}[1] SPI_NAND"
				echo "${space8}${space8}[2] SPI_NOR"
				read -p "Which would you like? [0~2][default:0]: " BM_INDEX
				MAX_BM_INDEX=2
			else
			...
			if __IS_IN_ARRAY "$HW_INDEX" "${range_powerfin[@]}" && (("$BM_INDEX" == 2)); then
				BM_INDEX=3 # SPI NOR
			fi
