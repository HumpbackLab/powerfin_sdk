...
do_build()
{
	...
	case "$1" in
		...
		kernel-modules | modules)
			...
			;;
		kernel*)
			KERNEL_DTB_TARGETS=("$RK_KERNEL_DTS_NAME.dtb")
			if [ -n "$RK_BOOT_FIT_RECOVERY_DTS_NAME" ]; then
				KERNEL_DTB_TARGETS+=(
			...
			if [ -n "$RK_BOOT_FIT_ITS" ] &&
				...
				# A FIT without a resource node does not need the vendor
				# %.img target. That target would create resource.img and an
				# intermediate boot/zboot image only to overwrite it below.
				run_command rm -f kernel/resource.img \
					kernel/boot.img kernel/zboot.img
				run_command $KMAKE "${KERNEL_DTB_TARGETS[@]}" \
					zImage modules
			else
			...
			if [ -n "$RK_BOOT_FIT_ITS" ] && \
				...
				if [ -n "$RK_BOOT_FIT_PACK_SCRIPT" ]; then
					FIT_PACK_SCRIPT="$RK_BOOT_FIT_PACK_SCRIPT"
					case "$FIT_PACK_SCRIPT" in
						/*) ;;
						*) FIT_PACK_SCRIPT="$RK_SDK_DIR/$FIT_PACK_SCRIPT" ;;
					esac
					if [ ! -x "$FIT_PACK_SCRIPT" ]; then
						error "FIT pack script is not executable: $FIT_PACK_SCRIPT"
						exit 1
					fi
					run_command "$FIT_PACK_SCRIPT" --output \
						"$RK_SDK_DIR/kernel/$RK_BOOT_IMG"
				elif grep -q '@RESOURCE_IMG@' "$RK_BOOT_FIT_ITS"; then
